...
function App() {
  ...
  const CopilotPage = () => {
    ...
    const riskScore = Math.min(100, Math.round(fatigue * 1.4 + pain * 1.5 + mood * 1.1));
    const riskLabel = riskScore >= 30 ? "Critical" : riskScore >= 18 ? "Needs review" : "Stable";

    const roleText = {
      patient: "I am a patient",
      caregiver: "I am a caregiver",
      support: "I am a support organization"
    }[role];
    ...
    return (
      <div className="copilot-page">
        <nav className="copilot-topbar">
          <button onClick={() => setPage("landing")}>← Welcome</button>
          <button onClick={() => setPage("map")}>Knowledge Graph</button>

          <div className="copilot-lang">
            <label>{t.langLabel}</label>
            <select value={lang} onChange={(e) => setLang(e.target.value)}>
              <option value="en">English</option>
              <option value="tr">Türkçe</option>
        ...
        <section className="copilot-hero">
          <div>
            <p className="copilot-kicker">SPLUNK AI HACKATHON PROJECT</p>
            <h1>OncoConnect AI Copilot</h1>
            <p>
              A patient support copilot that helps cancer patients, caregivers, and support teams
              turn symptom updates into care-ready guidance and Splunk-powered operational intelligence.
            </p>
          ...
          <div className="copilot-value-card">
            <h2>What value does it provide?</h2>
            <p>
              A user can select their situation, enter symptoms, receive a clear explanation,
              get doctor-visit questions, and send an event to Splunk so support teams can monitor high-risk cases.
            </p>
          </div>
        </section>

        <section className="copilot-kpis">
          <div><strong>15K+</strong><span>Operational events in Splunk</span></div>
          <div><strong>10K</strong><span>Chemotherapy records</span></div>
          <div><strong>5.2K</strong><span>Monitoring follow-ups</span></div>
          <div><strong>Live</strong><span>Live HEC event streaming</span></div>
        </section>

        <section className="copilot-workbench">
          <div className="copilot-form-card">
            <div className="copilot-step-title">
              <b>1</b>
              <div>
                <h2>Who are you?</h2>
            ...
            <div className="copilot-option-grid">
              <button className={role === "patient" ? "selected" : ""} onClick={() => setRole("patient")}>
                <strong>I am a patient</strong>
                <span>I want to understand my symptoms and prepare for my doctor visit.</span>
              </button>
              <button className={role === "caregiver" ? "selected" : ""} onClick={() => setRole("caregiver")}>
                <strong>I am a caregiver</strong>
                <span>I want to understand whether my loved one may need extra support.</span>
              </button>
              <button className={role === "support" ? "selected" : ""} onClick={() => setRole("support")}>
                <strong>I am a support organization</strong>
                <span>I want to prioritize high-risk patients for outreach.</span>
              </button>
            </div>
            ...
            <div className="copilot-option-grid">
              <button className={goal === "symptoms" ? "selected" : ""} onClick={() => setGoal("symptoms")}>
                <strong>Understand today’s symptoms</strong>
              </button>
              <button className={goal === "doctor" ? "selected" : ""} onClick={() => setGoal("doctor")}>
                <strong>Prepare questions for doctor visit</strong>
              </button>
              <button className={goal === "support" ? "selected" : ""} onClick={() => setGoal("support")}>
                <strong>Check whether extra support is needed</strong>
              </button>
            </div>
            ...
            <div className="copilot-step-title">
              ...
              <div>
                ...
                <p>Move the values to simulate a live support signal.</p>
              </div>
            </div>

            <div className="copilot-sliders">
              <label>
                <span>Fatigue</span>
          ...
          <div className="copilot-result-card">
            <div className="copilot-result-head">
              <div>
                <span>AI Copilot Result</span>
                <h2>{roleText}</h2>
                <p>{goalText}</p>
              </div>

              <div className={`copilot-risk ${riskLabel === "Critical" ? "critical" : ""}`}>
                <strong>{riskScore}</strong>
                <span>{riskLabel}</span>
              </div>
            </div>

            <div className="copilot-explain-box">
              <h3>What this means</h3>
              <p>
                Your symptom burden appears significantly elevated based on the values entered.
                This should be prioritized for care team review or support outreach.
              </p>
            </div>

            <div className="copilot-question-box">
              <h3>Questions to ask your doctor</h3>
              <ul>
                <li>Which symptoms should make me seek urgent support?</li>
                <li>Could fatigue or pain be related to treatment side effects?</li>
                <li>What should I track before my next visit?</li>
                <li>Who should my caregiver contact if symptoms increase?</li>
              </ul>
            </div>

            <div className="copilot-event-box">
              <span>Splunk event preview</span>
              <code>{`{ role: "${role}", goal: "${goal}", risk_score: ${riskScore}, status: "${riskLabel}" }`}</code>
            </div>

            <button className="copilot-send-btn">
              Send support event to Splunk
            </button>
          </div>
        </section>
      </div>
